...
with st.sidebar:
    st.header("ตั้งค่า")
    st.caption("หากต้องการใส่หรือเปลี่ยน Gemini API Key สามารถกรอกด้านล่าง:")
    ...
    sidebar_api_input = st.text_input(
        ...
        key="sidebar_api_key",
        help="กรอกเพื่ออัปเดต Gemini API Key ใหม่"
    )
    ...
    if sidebar_api_input and sidebar_api_input != st.session_state.gemini_api_key:
        ...
        st.toast("อัปเดต Gemini API Key เรียบร้อย", icon="🎉")
...
def get_api_key():
    ...
    st.caption("(ระบบจะบันทึกรหัสไว้จนกว่าจะรีเฟรชหน้าจอ)")
    api_input = st.text_input("Gemini API Key", type="password", key="dialog_api_input")
...
if not st.session_state.gemini_api_key:
    st.info("กรุณากรอก Gemini API Key เพื่อเริ่มใช้งานที่แถบด้านซ้าย")
...
try:
    ...
    st.error(f"Gemini API Key ไม่ถูกต้อง: {e}")
    st.stop()
...
def general_qna_interface():
    ...
    st.caption("""ข้อมูลการพยากรณ์อากาศอาจมีการเปลี่ยนแปลงตามสภาวะธรรมชาติ 
    ...
    system_instruction = (
        ...
        "**ที่สำคัญที่สุด:** หากผู้ใช้ถามเกี่ยวกับสภาพอากาศโดยไม่ระบุชื่ออำเภอ จังหวัด หรือสถานที่ (เช่น ถามว่า 'วันนี้ฝนตกไหม' เฉยๆ) ให้คุณตรวจสอบประวัติการสนทนาก่อนว่ามีการพูดถึงสถานที่ใดมาก่อนหน้านี้หรือไม่ ถ้ามีให้ใช้สถานที่นั้นตอบได้เลย ถ้ามันไม่มีก็ขอพื้นที่เขา "
    )
...
def weather_forecast_interface():
    ...
    data_source = st.radio(
        "เลือกวิธีการพยากรณ์:",
        ["AI พยากรณ์", "พยากรณ์ผ่านไฟล์ CSV/Excel"],
        horizontal=True
    )
    ...
    if data_source == "AI พยากรณ์":
        st.caption("""สามารถพยากรณ์สภาพอากาศล่วงหน้าได้สูงสุด 6 วัน เพื่อความแม่นยำของข้อมูล ข้อมูลที่แสดงผลเป็นข้อมูลของอำเภอที่ท่านเลือก
        ทางผู้พัฒนาขอสงวนสิทธิ์ในการรับผิดชอบต่อความคลาดเคลื่อนของข้อมูลที่อาจเกิดขึ้น เนื่องจากสภาพอากาศอาจมีการเปลี่ยนแปลงตามสภาวะธรรมชาติ ข้อมูลที่ปรากฏเป็นเพียงการพยากรณ์เบื้องต้นเท่านั้น""")
        ...
        with st.form(key="required_info"):
            ...
            day_options = ["วันนี้", "พรุ่งนี้", "2 วันข้างหน้า", "3 วันข้างหน้า", "4 วันข้างหน้า","5 วันข้างหน้า","6 วันข้างหน้า"]
        ...
        if submit_button:
            ...
            if not selected_province or not selected_district:
                st.warning("กรุณาระบุ จังหวัด และ อำเภอ/เขต ให้ครบถ้วน")
            else:
                st.success(f"คุณเลือกจังหวัด: {selected_province}  อำเภอ/เขต: {selected_district}")
                st.info(f"คุณเลือกวันที่: {custom_date.strftime('%d-%m-%Y')}")
    ...
    if weather_df is not None:
        ...
        aqi_domain = ["ดีมาก (0-25)", "ดี (26-50)", "ปานกลาง (51-100)", "เริ่มมีผลกระทบต่อสุขภาพ (101-200)", "มีผลกระทบต่อสุขภาพ (201 ขึ้นไป)"]
        aqi_range = ["#00C896", "#8CD600", "#F9D600", "#FF9600", "#E60000"] # สีที่ปรับปรุงใหม่

        aqi_map = {val: idx for idx, val in enumerate(aqi_domain)}
        weather_df['AQI_Index'] = weather_df['AQI'].map(aqi_map).fillna(0)
        ...
        with col2:
            ...
            with st.container(border=True):
                ...
                st.metric("สภาพท้องฟ้าโดยรวม", sky_mode)
